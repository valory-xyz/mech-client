...
def send_safe_tx(  # pylint: disable=too-many-arguments
    ...
    tx_data: str,
    to_adress: str,  # Note: typo preserved for backward compatibility
    safe_address: str,
    signer_pkey: str,
    value: int = 0,
) -> Optional[HexBytes]:
    """Send a Safe transaction (legacy function).

    Deprecated: Use SafeClient class instead.

    :param ethereum_client: Ethereum client instance
    :param tx_data: Transaction data (hex)
    :param to_adress: Destination address (note: typo preserved)
    :param safe_address: Safe address
    :param signer_pkey: Signer private key
    :param value: Value in wei
    :return: Transaction hash if successful
    """
    client = SafeClient(ethereum_client, safe_address)
...
def get_safe_nonce(ethereum_client: EthereumClient, safe_address: str) -> int:
    """Get the Safe nonce (legacy function).

    Deprecated: Use SafeClient.get_nonce() instead.
